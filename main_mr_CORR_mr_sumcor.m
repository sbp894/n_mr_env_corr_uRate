...
parfor plotVar=1:size(unique_chin_snr_track_unit_mat,1)
    ...
    for indVar=1:length(cur_inds)
        ...
        if strcmp(curMetaData.noise, 'FLN')
            [mr_corr_Data(plotVar).FLNcorr, mr_corr_Data(plotVar).FLNcorrNF]=multires_modulation_sumcor_anal(S_rate_plus, S_rate_minus, N_rate_plus, N_rate_minus, ...
                SN_rate_plus, SN_rate_minus, 1/meanrate_binwidth, AllmodFreqs);
        elseif strcmp(curMetaData.noise, 'SSN')
            [mr_corr_Data(plotVar).SSNcorr, mr_corr_Data(plotVar).SSNcorrNF]=multires_modulation_sumcor_anal(S_rate_plus, S_rate_minus, N_rate_plus, N_rate_minus, ...
                SN_rate_plus, SN_rate_minus, 1/meanrate_binwidth, AllmodFreqs);
        end
...
fName_singleModFreqCorr=sprintf('sEPSM_sumcor_modFreq%.0fHz', singleModFreq);
if saveAllFigs
