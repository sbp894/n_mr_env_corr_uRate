...
parfor plotVar=1:size(unique_chin_snr_track_unit_mat,1)
    ...
    for indVar=1:length(cur_inds)
        ...
        if strcmp(curMetaData.noise, 'FLN')
            [mr_corr_Data(plotVar).FLNcorr, mr_corr_Data(plotVar).FLNcorrNF]=multires_modulation_corr_anal(S_rate_env, SN_rate_env, N_rate_env, 1/meanrate_binwidth, AllmodFreqs);
        elseif strcmp(curMetaData.noise, 'SSN')
            [mr_corr_Data(plotVar).SSNcorr, mr_corr_Data(plotVar).SSNcorrNF]=multires_modulation_corr_anal(S_rate_env, SN_rate_env, N_rate_env, 1/meanrate_binwidth, AllmodFreqs);
        end
...
saveas(hanAllModFreqCorr, [saving_Dir 'allModFreq_sEPSMcorr'], 'tiff');
saveas(hanAllModFreqCorr, [Latex_Dir 'sEPSMcorr_allModFreq'], 'epsc');
...
fName_singleModFreqCorr=sprintf('sEPSMcorr_modFreq%.0fHz', singleModFreq);
