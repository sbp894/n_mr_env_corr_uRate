function [corr_s_sn_ValsModFreq, uncorr_sn_n_ValsModFreq, corr_s_n_ValsModFreqNF]=...
    multires_modulation_tfs_hilb_mod_fbank(S_rate_plus_Org, S_rate_minus_Org, N_rate_plus_Org, N_rate_minus_Org, SN_rate_plus_Org, SN_rate_minus_Org, fsOrg, modFreq)
...
corr_s_n_ValsModFreqNF=nan(length(modFreq),1);
...
powerModS=nan(length(modFreq), 1);
...
for modFreqVar=1:length(modFreq)
    ...
    powerModS(modFreqVar)=var(S_rate_tfs_filt);
    powerModSN(modFreqVar)=var(SN_rate_tfs_filt);
    powerModN(modFreqVar)=var(N_rate_tfs_filt);
    ...
    uncur_sn_n_CorrVals=1-cur_sn_n_CorrVals;
    cur_s_n_CorrVals(cur_s_n_CorrVals<0)=0;
   
    corr_s_sn_ValsModFreq(modFreqVar)=sqrt(nanmean(cur_s_sn_CorrVals.^2));
    uncorr_sn_n_ValsModFreq(modFreqVar)=sqrt(nanmean(uncur_sn_n_CorrVals.^2));
    corr_s_n_ValsModFreqNF(modFreqVar)=sqrt(nanmean(cur_s_n_CorrVals.^2));
end