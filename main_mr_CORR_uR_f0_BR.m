...
outFigDir.pdf=sprintf('/media/parida/DATAPART1/Matlab/SNRenv/n_mr_env_corr_uRate/outFig/uR_f0_BR_%d/pdf/', modFreq);
if ~isdir(outFigDir.pdf)
    mkdir(outFigDir.pdf);
end
outFigDir.png=sprintf('/media/parida/DATAPART1/Matlab/SNRenv/n_mr_env_corr_uRate/outFig/uR_f0_BR_%d/png/', modFreq);
...
%% Main parfor loop
for plotVar=18 %1:size(unique_chin_snr_track_unit_mat,1)
    ...
    for indVar=1:length(cur_inds)
        ...
        if strcmp(curMetaData.noise, 'FLN')
            [mr_corr_Data(plotVar).FLNcorr_s_sn_pos, mr_corr_Data(plotVar).FLNuncorr_sn_n_pos, mr_corr_Data(plotVar).FLNcorr_s_n_pos, ...
                ...
                multires_modulation_uR_f0_BR(S_rate_plus, S_rate_minus, N_rate_plus, N_rate_minus, ...
                SN_rate_plus, SN_rate_minus, 1/meanrate_binwidth, modFreq, outFigDir, TimeResolution, combine_chi1_mu0, N_bp, [figName '_FLN']);
        elseif strcmp(curMetaData.noise, 'SSN')
            [mr_corr_Data(plotVar).SSNcorr_s_sn_pos, mr_corr_Data(plotVar).SSNuncorr_sn_n_pos, mr_corr_Data(plotVar).SSNcorr_s_n_pos, ...
                ...
                multires_modulation_uR_f0_BR(S_rate_plus, S_rate_minus, N_rate_plus, N_rate_minus, ...
                SN_rate_plus, SN_rate_minus, 1/meanrate_binwidth, modFreq, outFigDir, TimeResolution, combine_chi1_mu0, N_bp, [figName '_SSN']);
        end
...
fName2Save=sprintf('%suRate_uR_f0_BR_%d.mat', saving_Dir, modFreq);
...
if combine_chi1_mu0
    ...
    yyaxis left, ylabel('$\overline{R_{f0,BR}(S).R_{f0,BR}(SN)}$', 'interpreter', 'latex');
    ylim([0 1]);
end
...
fName_singleModFreqCorr=sprintf('sEPSM_uR_f0_BR_modFreq%.0fHz_corr_%s_tRes%.0fms', modFreq, postfix_fName, TimeResolution*1e3);
if saveAllFigs
...
if combine_chi1_mu0
    ...
    yyaxis left, ylabel('$\overline{1-R_{f0,BR}(SN).R_{f0,BR}(S)}$', 'interpreter', 'latex');
    ylim([0 1]);
end
yyaxis right; ylabel('d-prime');
...
fName_singleModFrequnCorr=sprintf('sEPSM_uR_f0_BR_modFreq%.0fHz_uncorr_%s_tRes%.0fms', modFreq, postfix_fName, TimeResolution*1e3);
