...
Chin_Track_Unit_subset= [[361 1 21]; [361 1 5]; [361 2 4]; [355 4 12]; [355 4 5]; [338 2 3]];
...
outFigDir.fig=sprintf('/media/parida/DATAPART1/Matlab/SNRenv/n_mr_env_corr_uRate/outFig/uR_f0_BP_%d/fig/', modFreq);
...
for plotVar= 1:size(unique_chin_snr_track_unit_mat,1)
    if ismember(unique_chin_snr_track_unit_mat(plotVar, [1 3 4]), Chin_Track_Unit_subset, 'rows')
        ...
        for indVar=1:length(cur_inds)
            ...
            if strcmp(curMetaData.noise, 'FLN')
                ...
                saveas(gcf, [outFigDir.fig [figName '_FLN']]);
            elseif strcmp(curMetaData.noise, 'SSN')
                ...
                saveas(gcf, [outFigDir.fig [figName '_SSN']]);
            end
...
% % % % %     yyaxis left, ylabel('$\overline{\chi ^2}$', 'interpreter', 'latex');
% % % % % else
% % % % %     yyaxis left, ylabel('$1-\overline{R_{f0,BP}(SN).R_{f0,BP}(S)}$', 'interpreter', 'latex');
% % % % %     ylim([0 1]);
% % % % % end
% % % % % yyaxis right; ylabel('d-prime');
...
% % % % % fName_singleModFrequnCorr=sprintf('sEPSM_uR_f0_BP_modFreq%.0fHz_uncorr_%s_tRes%.0fms', modFreq, postfix_fName, TimeResolution*1e3);
% % % % %
% % % % % if saveAllFigs
