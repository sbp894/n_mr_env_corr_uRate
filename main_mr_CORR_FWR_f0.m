...
outFigDir.pdf=sprintf('/media/parida/DATAPART1/Matlab/SNRenv/n_mr_env_corr_uRate/outFig/FWR_f0_%d/pdf/', modFreq);
if ~isdir(outFigDir.pdf)
    mkdir(outFigDir.pdf);
end
outFigDir.png=sprintf('/media/parida/DATAPART1/Matlab/SNRenv/n_mr_env_corr_uRate/outFig/FWR_f0_%d/png/', modFreq);
...
parfor plotVar=1:size(unique_chin_snr_track_unit_mat,1)
    ...
    for indVar=1:length(cur_inds)
        ...
        if strcmp(curMetaData.noise, 'FLN')
            [mr_corr_Data(plotVar).FLNcorr_s_sn, mr_corr_Data(plotVar).FLNuncorr_sn_n, mr_corr_Data(plotVar).FLNcorr_s_n]=multires_modulation_FWR_f0(S_rate_plus, S_rate_minus, N_rate_plus, N_rate_minus, ...
                SN_rate_plus, SN_rate_minus, 1/meanrate_binwidth, modFreq, outFigDir, [figName '_FLN']);
        elseif strcmp(curMetaData.noise, 'SSN')
            [mr_corr_Data(plotVar).SSNcorr_s_sn, mr_corr_Data(plotVar).SSNuncorr_sn_n, mr_corr_Data(plotVar).SSNcorr_s_n]=multires_modulation_FWR_f0(S_rate_plus, S_rate_minus, N_rate_plus, N_rate_minus, ...
...
fName2Save=sprintf('%suRate_FWR_f0_%d.mat', saving_Dir, modFreq);
...
fName_singleModFreqCorr=sprintf('sEPSM_FWR_f0_modFreq%.0fHz_corr', modFreq);
if saveAllFigs
...
fName_singleModFrequnCorr=sprintf('sEPSM_FWR_f0_modFreq%.0fHz_uncorr', modFreq);
if saveAllFigs
